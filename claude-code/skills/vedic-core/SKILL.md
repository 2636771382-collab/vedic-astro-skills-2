...
5. **写入限制**：每次write_to_file控制在400行以内。超过则分次追加。
...
如果structured_data中"核心关切"或"具体问题"非空：

**在Step 1之前，先写一段"优先回应"**（写入p2a_planets.md开头）：
```markdown
## 🎯 您最关心的问题

> 您的问题：[用户原话]

[2-3段直接回应，基于快速扫描关键宫位/Dasha]

> 详细分析见后文[板块X]。以下是完整的系统审计。
```

**要点**：
- 不用等50页报告，第一页就给答案
- 回应基于数据（哪个宫主/Dasha/格局），不是空话
- 标注"详见板块X"引导深读
- 如果关切=事业 → 快速看10宫+AmK+当前大运
- 如果关切=感情 → 快速看7宫+DK+Venus+当前大运
- 如果关切=财运 → 快速看2宫+11宫+SAV+D4
- 如果没有核心关切 → 跳过此段，直接Step 1
...
第1段：这颗星管你人生的什么事？
  ...
  → 跟用户的实际经历挂钩："所以你从XX岁开始就跟'出国'这件事绑在一起"
...
```
"你的10宫主是[行星]，Shadbala [X]%([强/中/弱])，
 落在[Y]宫。在[大运行星]大运期间（YYYY-YYYY），
 你的事业领域应该经历了[描述]..."
```
...
⚠️ 至少写2-3段，像在跟用户解释他家的某个"房间"：
  第1段：这个宫位管你人生的什么事？管家（宫主）去哪了？状态好不好？
    → "管你婚姻的那颗星（Venus）跑到了9宫去——它不在家，
       但还惦记着家里的事"
  第2段：实际影响是什么？跟用户的经历挂钩
    → "所以你到现在还没遇到特别认真的感情——不是你的问题，
       是管家不在位"
  第3段（重点宫位）：Dasha事件关联，用具体年份
    → "2028-2031年期间，这个宫位会被激活..."
